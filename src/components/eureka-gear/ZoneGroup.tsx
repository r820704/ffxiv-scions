...
export function ZoneGroup({ zone, items, materialsMap }: ZoneGroupProps) {
  ...
  return (
    <section className="mb-3">
      ...
      {!allDone && (
        <ul className="bg-gray-900 rounded p-3 text-sm grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-4 gap-y-1">
          {items.map((it) => {
            const meta = materialsMap[it.materialId];
            const name = meta?.nameTC ?? `#${it.materialId}`;
            const icon = MATERIAL_ICONS[meta?.icon ?? 0];
            return (
              <li key={it.materialId} className="flex items-center justify-between gap-2 min-w-0">
                <span className="flex items-center gap-2 min-w-0">
                  {icon && <img src={icon} alt="" aria-hidden="true" className="w-5 h-5 shrink-0" loading="lazy" />}
                  <span className="truncate">{name}</span>
                </span>
                <span className={`shrink-0 tabular-nums ${it.shortage > 0 ? 'text-red-400' : 'text-green-400'}`}>

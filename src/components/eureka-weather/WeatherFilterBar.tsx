...
export default function WeatherFilterBar({ selected, onToggle, onClearAll }: WeatherFilterBarProps) {
    ...
    <div className="flex flex-wrap items-center gap-1.5 bg-secondary rounded-lg p-3">
      ...
      {hasSelection && onClearAll && (
        <button
          type="button"
          onClick={onClearAll}
          className="ml-auto flex items-center gap-1 text-xs px-2 py-0.5 rounded border border-border/50 text-muted-foreground hover:border-primary hover:text-primary transition-colors cursor-pointer"
        >
          <span>✕</span>

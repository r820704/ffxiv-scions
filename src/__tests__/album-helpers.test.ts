...
describe('MNEME_FIXED_ORDER', () => {
  it('should contain all mneme IDs from eurekaData', () => {
    const allMnemeIds = eurekaData.mnemes.map((m) => m.id);
    for (const id of MNEME_FIXED_ORDER) {
      expect(allMnemeIds).toContain(id);
    }
  });
});
